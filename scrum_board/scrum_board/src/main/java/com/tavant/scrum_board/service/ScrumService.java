...
public class ScrumService {
	...
	public Scrum getScrum(String scrumName) {

		return scrumRepositories.findByScrumName(scrumName);

	}

	public void deleteScrum(String scrumName) {
		...
		int lengthOfScrum = scrumTasks.size();
		int[] arrayOftasksId = new int[100];
		int localVal = 0;
		for (Task task : scrumTasks) {

			arrayOftasksId[localVal] = task.getTaskid();
			localVal++;

		}
		scrumRepositories.deleteByScrumName(scrumName);

		for (int i = lengthOfScrum - 1; i >= 0; i--) {

			taskRepositories.deleteById(arrayOftasksId[i]);
		}
